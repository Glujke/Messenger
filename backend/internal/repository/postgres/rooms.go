...
// ListUserRooms returns direct rooms for the given user with peer details.
func (s *Store) ListUserRooms(ctx context.Context, userID int64) ([]repository.RoomListRecord, error) {
	const query = `
		SELECT r.id, u.id, u.email
		FROM rooms r
		JOIN room_members me ON me.room_id = r.id AND me.user_id = $1
		JOIN room_members peer ON peer.room_id = r.id AND peer.user_id <> $1
	...
	for rows.Next() {
		...
		if err := rows.Scan(&record.ID, &record.PeerID, &record.PeerEmail); err != nil {
			return nil, err
		}
